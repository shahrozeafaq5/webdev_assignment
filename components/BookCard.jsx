...
import { fadeInUp } from "@/lib/animations"; // Use shared animation

export default function BookCard({ book }) {
  return (
    // Wrap the card in motion.div and assign the variant
    // This allows the parent 'staggerContainer' to trigger it
      ...
      <Link href={`/books/${book._id}`} className="group block cursor-pointer">
        ...
        <div className="flex justify-between items-start gap-4">
          ...
          <span className="text-[10px] font-bold uppercase tracking-wider text-stone-400 border border-stone-200 px-2 py-1 rounded-sm">
            {book.category ? book.category.slice(0, 4) : "GEN"}
          </span>
        </div>
      </Link>
    </motion.div>
